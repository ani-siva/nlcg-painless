...
def contour(X,Y,Z,X1,X2,X3,X4):
    ...
    plt.plot(X1,X2,'bo-',label='Steepest Descent')
    plt.plot(X3,X4,'r.-',label='Jacobi Iterations')
...
with open("steep-coords.txt") as datafile:
    reader=csv.reader(datafile,delimiter=' ')
    for row in reader:
        x1.append(float(row[0]))
        x2.append(float(row[1]))
with open("jacobi-iter-coords.txt") as datafile:
    reader=csv.reader(datafile,delimiter=' ')
    for row in reader:
